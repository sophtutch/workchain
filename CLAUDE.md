...
├── exceptions.py               — Exception hierarchy: WorkchainError, StepError, StepTimeoutError, HandlerError, LockError, FenceRejectedError, RecoveryError
└── contrib/
...
- **Auto-population of Step fields from handler meta**: At workflow construction time (PENDING workflows only), `Workflow._resolve_and_validate_depends_on` copies decorator-declared values (`retry`, `poll`, `is_async`, `completeness_check`, `idempotent`, `depends_on`) onto the `Step` model when the caller did not explicitly pass them. "Explicitly passed" is detected via Pydantic's `model_fields_set`, so `default_factory` defaults do not block the copy. Mongo reloads are not mutated (guarded by `status == PENDING`). Templates and designer drafts still win when they pass a policy explicitly — they just don't fight the decorator default when they leave a field unset.
